...
ENV_NAME=rsinference
ENV_ROOT=$CONDA_ROOT/envs/$ENV_NAME

SENTINEL_FILE=${WORKSPACE_DIR}/.env_setup_finished_rsinference
...
if [[ ! -f $SENTINEL_FILE ]]; then
  ...
  if [[ $OS == "Linux" ]]; then
    ...
    # On aarch64 there is usually no compatible pip wheel named pyrealsense2.
    # conda-forge's librealsense package includes the Python module
    # `pyrealsense2` for python=3.10 on linux-aarch64 and linux-64.
    MAMBA_ROOT_PREFIX=$CONDA_ROOT $CONDA_ROOT/bin/mamba install -y -n $ENV_NAME \
      -c conda-forge --override-channels librealsense
    python - <<'PY'
import pyrealsense2 as rs
ctx = rs.context()
print(f"pyrealsense2 import OK; detected {len(ctx.devices)} RealSense device(s)")
PY
  fi
