...
def _resolve_object_urdf_for_key(object_urdf_folder: str, object_key: str) -> str:
    """Resolve a URDF for object key from folder with common layouts."""
    candidates = [
...
def _discover_motion_npz_files(motion_folder: str) -> list[str]:
    return sorted(str(path) for path in Path(motion_folder).rglob("*.npz"))


def _resolve_multi_object_urdf_config(tyro_config: ExperimentConfig) -> ExperimentConfig:
    ...
    if motion_folder:
        ...
        motion_files = _discover_motion_npz_files(resolved_motion_folder)
        if not motion_files:
    ...
    object_keys = sorted({_extract_object_key_from_motion_name(mf) for mf in motion_files})
    object_name_to_path = {key: _resolve_object_urdf_for_key(resolved_folder, key) for key in object_keys}
